...
class RSCoTrSegPixelDecoder(BaseModule):
    ...
    def __init__(self,
        ...
        if isinstance(encoder, BaseModule):
            self.num_encoder_levels = \
                encoder.layers[0].attentions[0].num_levels - 1
        else:
            self.num_encoder_levels = \
        ...
        self.input_convs, init_requirement['input_convs'] = \
            self.init_input_convs(
            in_channels, feat_channels, norm_cfg, False, input_convs)
        self.encoder, init_requirement['encoder'] = \
            build_transformer_layer_sequence(encoder)
        ...
        self.init_requirement = init_requirement

    def init_input_convs(self, in_channels, feat_channels, norm_cfg, bias=True,
                         input_convs=None):
        if input_convs is not None:
            assert isinstance(input_convs, (nn.ModuleList, ModuleList))
            assert len(input_convs) == self.num_encoder_levels
            return input_convs, False
        input_conv_list = []
        # from top to down (low to high resolution)
        ...
        return ModuleList(input_conv_list), True

    def init_weights(self):
        """Initialize weights."""
        if self.init_requirement['input_convs']:
        ...
        if self.init_requirement['encoder']:
